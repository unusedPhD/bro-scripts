#Source: https://github.com/securitykitten/bro_scripts.git
...
function calculate_entropy(host: addr, server: addr): double
    ...
    for (i in collection)
        ...
        if ( i+1 >= length )
            ...
            #intervals[i] = double_to_interval(double_to_count(interval_to_double(collection[i+1] - collection[i]))); #wtf is this
